...
function yourls_create_sqlite_tables() {
    ...
    // Create indexes
    $ydb->perform('CREATE INDEX [timestamp] ON ['.YOURLS_DB_TABLE_URL.']([timestamp])');
    $ydb->perform('CREATE INDEX [ip] ON ['.YOURLS_DB_TABLE_URL.']([ip])');
    $ydb->perform('CREATE INDEX [shorturl] ON ['.YOURLS_DB_TABLE_LOG.']([shorturl])');
